...
static short burm_nts_3[] = { burm_con_NT, burm_con_NT, 0 };
...
void burm_reduce(NODEPTR_TYPE bnode, int goalnt)
{
 ...
 printf("movq %s, %%rax\nret\n", kids[0]->regname); FREEREG(1)
    break;
  case 7:
 bnode->val = kids[0]->val + kids[1]->val;
    break;
  case 8:
 NEWREG(bnode) printf("movq %s, %s\naddq %s, %s\n", kids[0]->regname, bnode->regname, kids[1]->regname, bnode->regname); FREEREG(2)
    break;
  default:    assert (0);
  }
}
